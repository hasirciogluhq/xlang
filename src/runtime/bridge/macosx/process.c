...
const char* xl_capture_stdout(void) {
...
int32_t xl_proc_wait(int32_t pid) {
    int status = 0;
...
void xl_proc_exit(int32_t code) {
    _exit(code);
}

int64_t xl_pipe_create(void) {
    int pipe_fds[2];
    if (pipe(pipe_fds) != 0) {
        return -1;
    }
...
int32_t xl_fd_close(int32_t fd) {
    return close(fd);
}

const char* xl_env_get(const char* key) {
    if (key == NULL) {
        return "";
    }
...
int32_t xl_chdir(const char* path) {
    if (path == NULL) {
        return 1;
    }
    return chdir(path);
...
int32_t xl_pipe_read_fd(int64_t handle) {
    return (int32_t)(handle >> 32);
}

int32_t xl_pipe_write_fd(int64_t handle) {
