...
int32_t filesystem_errno(void) {
    return (int32_t)g_fs_errno;
}

int64_t filesystem_open(const char* path, int32_t mode) {
    fs_set_err(0);
...
int32_t filesystem_close(int64_t handle) {
    fs_set_err(0);
    if (handle <= 0 || handle > FS_MAX_HANDLES) {
        fs_set_err(EBADF);
        return 1;
...
const char* filesystem_read(int64_t handle, int32_t max) {
    fs_set_err(0);
...
int32_t filesystem_write(int64_t handle, const char* data) {
    fs_set_err(0);
    FILE* fp = fs_fp(handle);
    if (fp == NULL) {
        fs_set_err(EBADF);
...
int32_t filesystem_write_n(int64_t handle, const char* data, int32_t n) {
    fs_set_err(0);
    FILE* fp = fs_fp(handle);
    if (fp == NULL) {
        fs_set_err(EBADF);
...
int64_t filesystem_seek(int64_t handle, int64_t offset, int32_t whence) {
    fs_set_err(0);
    FILE* fp = fs_fp(handle);
    if (fp == NULL) {
        fs_set_err(EBADF);
...
int64_t filesystem_tell(int64_t handle) {
    fs_set_err(0);
    FILE* fp = fs_fp(handle);
    if (fp == NULL) {
        fs_set_err(EBADF);
...
int32_t filesystem_flush(int64_t handle) {
    fs_set_err(0);
    FILE* fp = fs_fp(handle);
    if (fp == NULL) {
        fs_set_err(EBADF);
...
int32_t filesystem_truncate(int64_t handle, int64_t size) {
    fs_set_err(0);
    FILE* fp = fs_fp(handle);
    if (fp == NULL) {
        fs_set_err(EBADF);
...
const char* filesystem_read_all(const char* path) {
    fs_set_err(0);
    if (path == NULL || path[0] == '\0') {
        fs_set_err(EINVAL);
        return fs_empty();
...
int32_t filesystem_write_all(const char* path, const char* data) {
    fs_set_err(0);
    if (path == NULL || path[0] == '\0') {
        fs_set_err(EINVAL);
        return 1;
...
int32_t filesystem_append_all(const char* path, const char* data) {
    fs_set_err(0);
    if (path == NULL || path[0] == '\0') {
        fs_set_err(EINVAL);
        return 1;
...
int32_t filesystem_exists(const char* path) {
    fs_set_err(0);
    if (path == NULL || path[0] == '\0') {
        return 0;
    }
...
int32_t filesystem_is_file(const char* path) {
    fs_set_err(0);
    if (path == NULL || path[0] == '\0') {
        return 0;
    }
...
int32_t filesystem_is_dir(const char* path) {
    fs_set_err(0);
    if (path == NULL || path[0] == '\0') {
        return 0;
    }
...
int32_t filesystem_is_symlink(const char* path) {
    fs_set_err(0);
    if (path == NULL || path[0] == '\0') {
        return 0;
    }
...
int64_t filesystem_size(const char* path) {
    fs_set_err(0);
    if (path == NULL || path[0] == '\0') {
        fs_set_err(EINVAL);
        return -1;
...
int64_t filesystem_mtime(const char* path) {
    fs_set_err(0);
    if (path == NULL || path[0] == '\0') {
        fs_set_err(EINVAL);
        return -1;
...
int32_t filesystem_mkdir(const char* path) {
    fs_set_err(0);
    if (path == NULL || path[0] == '\0') {
        fs_set_err(EINVAL);
        return 1;
...
int32_t filesystem_mkdir_all(const char* path) {
    fs_set_err(0);
    if (path == NULL || path[0] == '\0') {
        fs_set_err(EINVAL);
        return 1;
...
int32_t filesystem_remove(const char* path) {
    fs_set_err(0);
    if (path == NULL || path[0] == '\0') {
        fs_set_err(EINVAL);
        return 1;
...
int32_t filesystem_rmdir(const char* path) {
    fs_set_err(0);
    if (path == NULL || path[0] == '\0') {
        fs_set_err(EINVAL);
        return 1;
...
int32_t filesystem_rename(const char* from, const char* to) {
    fs_set_err(0);
...
int32_t filesystem_copy(const char* from, const char* to) {
    fs_set_err(0);
    if (from == NULL || to == NULL || from[0] == '\0' || to[0] == '\0') {
        fs_set_err(EINVAL);
        return 1;
...
int32_t filesystem_chmod(const char* path, int32_t mode) {
    fs_set_err(0);
    if (path == NULL || path[0] == '\0') {
        fs_set_err(EINVAL);
        return 1;
...
const char* filesystem_cwd(void) {
    fs_set_err(0);
...
int32_t filesystem_chdir(const char* path) {
    fs_set_err(0);
    if (path == NULL || path[0] == '\0') {
        fs_set_err(EINVAL);
        return 1;
...
const char* filesystem_realpath(const char* path) {
    fs_set_err(0);
    if (path == NULL || path[0] == '\0') {
        fs_set_err(EINVAL);
        g_path_buffer[0] = '\0';
...
const char* filesystem_read_dir(const char* path) {
    fs_set_err(0);
    if (path == NULL || path[0] == '\0') {
        fs_set_err(EINVAL);
        return fs_empty();
...
const char* filesystem_read_handle(int64_t handle) {
    fs_set_err(0);
    FILE* fp = fs_fp(handle);
    if (fp == NULL) {
        fs_set_err(EBADF);
...
int32_t filesystem_write_handle(int64_t handle, const char* data) {
    const int32_t n = filesystem_write(handle, data);
    if (n < 0) {
        return 1;
    }
    return filesystem_flush(handle);
}
