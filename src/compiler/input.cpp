...
BuildKind defaultBuildKind(const InputKind) {
...
std::filesystem::path defaultOutputPath(const std::filesystem::path& input,
    ...
    case BuildKind::Object:
        return parent / (stem + platform::objectSuffix(os));
    case BuildKind::Static:
        return parent / (platform::staticLibPrefix(os) + stem + platform::staticLibSuffix(os));
    case BuildKind::Shared:
        return parent / (platform::staticLibPrefix(os) + stem + platform::sharedLibSuffix(os));
    case BuildKind::Executable:
    default:
        if (input_kind == InputKind::Object || input_kind == InputKind::LlvmIr ||
            input_kind == InputKind::Xlang) {
            return parent / (stem + platform::exeSuffix(os));
        }
        return parent / (stem + platform::exeSuffix(os));
    }
}
